...
// ── useTweaks ───────────────────────────────────────────────────────────────
function useTweaks<T extends Record<string, any>>(defaults: T): [T, (keyOrEdits: keyof T | Partial<T>, val?: any) => void] {
...
function TweakRadio({ label, value, options, onChange }: TweakRadioProps) {
  ...
  const labelLen = (o: TweakRadioOption | any) => String(typeof o === 'object' ? o.label : o).length
...
function TweakNumber({ label, value, min, max, step = 1, unit = '', onChange }: TweakNumberProps) {
  ...
  return (
    <div className="twk-num">
      <span className="twk-num-lbl" onPointerDown={onScrubStart as any}>{label}</span>
...
const __TwkCheck: React.FC<TwkCheckProps> = ({ light }) => (
